...
SUSUWU_COMPILER_COMMAND() { #/* Usage: `SUSUWU_COMPILER_COMMAND "<directory>" "<file>" ["<command>" | <arguments>...]"`. Allows to echo commands ( + produce `./${SUSUWU_COMPILER_JSON_PATH}`). */
	...
	if SUSUWU_HAS_PARTIAL_JSON "${SUSUWU_COMPILER_JSON_PATH_}" && [ "/dev/null" = "${SUSUWU_COMPILER_JSON_PATH}" ]; then
		SUSUWU_PRINT "SUSUWU_HAS_PARTIAL_JSON()" "$(SUSUWU_SH_INFO)" "found partial $(SUSUWU_SH_QUOTE "PATH" "${SUSUWU_COMPILER_JSON_PATH_}"), will resume."
		export SUSUWU_COMPILER_JSON_PATH="${SUSUWU_COMPILER_JSON_PATH_}"
	fi
	{
		echo "	{"
		echo "		\"directory\": \"${1}\","
...
SUSUWU_SETUP_OBJDIR() { #/* Usage: `SUSUWU_SETUP_OBJDIR "./obj/"` */
	if [ -z ${OBJDIR} ]; then
		OBJDIR="${1}"
		SUSUWU_SETUP_OBJDIR_OLD="$(SUSUWU_SH_QUOTE "PATH CURRENT" "${OBJDIR}")"
		SUSUWU_SETUP_OBJDIR_NEW="$(SUSUWU_SH_QUOTE "PATH PROPOSED" "<new-path>")"
...
SUSUWU_SETUP_BINDIR() { #/* Usage: `SUSUWU_SETUP_BINDIR "./bin/"` */
	if [ -z ${BINDIR} ]; then
		BINDIR="${1}"
		SUSUWU_SETUP_BINDIR_OLD="$(SUSUWU_SH_QUOTE "PATH CURRENT" "${BINDIR}")"
		SUSUWU_SETUP_BINDIR_NEW="$(SUSUWU_SH_QUOTE "PATH PROPOSED" "<new-path>")"
