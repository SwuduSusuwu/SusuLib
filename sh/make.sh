...
#			CXXFLAGS="${CXXFLAGS} -rtlib=compiler-rt"
			...
			SUSUWU_PRINT "SUSUWU_SETUP_CXX()" "$(SUSUWU_SH_ERROR)" "$(SUSUWU_SH_CODE_QUOTE "x86_64-w64-mingw32-clang++ not found"), $(SUSUWU_SH_CODE_QUOTE "x86_64-w64-mingw32-g++ not found"). Do $(SUSUWU_SH_CODE_QUOTE "apt install llvm-mingw-w64") or $(SUSUWU_SH_CODE_QUOTE "apt install mingw-w64")."
			exit 1
		fi
		...
		SUSUWU_PRINT "SUSUWU_SETUP_CXX()" "$(SUSUWU_SH_INFO)" "$(SUSUWU_SH_CODE_QUOTE "clang++ not found"), $(SUSUWU_SH_CODE_QUOTE "g++ not found"). $(SUSUWU_SH_CODE_QUOTE "CXX=${CXX}") found, will use."
		if command -v "${CC}" >/dev/null; then
			SUSUWU_PRINT "SUSUWU_SETUP_CXX()" "$(SUSUWU_SH_INFO)" "$(SUSUWU_SH_CODE_QUOTE "CC=${CC}") found, will use."
		else
			SUSUWU_PRINT "SUSUWU_SETUP_CXX()" "$(SUSUWU_SH_INFO)" "$(SUSUWU_SH_CODE_QUOTE "CC=${CC}") not found, will use $(SUSUWU_SH_CODE_QUOTE "CC=\"\${CXX} -x -c\"")"
			CC="${CXX}"
			CFLAGS="${CFLAGS} -x c"
		fi
		if command -v "${LD}" >/dev/null; then
			SUSUWU_PRINT "SUSUWU_SETUP_CXX()" "$(SUSUWU_SH_INFO)" "$(SUSUWU_SH_CODE_QUOTE "LD=${LD}") found, will use."
		else
			SUSUWU_PRINT "SUSUWU_SETUP_CXX()" "$(SUSUWU_SH_INFO)" "$(SUSUWU_SH_CODE_QUOTE "LD=${LD}") not found, will use $(SUSUWU_SH_CODE_QUOTE "LD=\"\${CXX}\"")"
			LD="${CXX}"
		fi
		USE_FSAN=false #/* `TODO: test unknown compilers for `-fsan*` support */
		return 0
	else
		SUSUWU_PRINT "SUSUWU_SETUP_CXX()" "$(SUSUWU_SH_ERROR)" "$(SUSUWU_SH_CODE_QUOTE "clang++ not found"), $(SUSUWU_SH_CODE_QUOTE "g++ not found"). $(SUSUWU_SH_CODE_QUOTE "CXX=${CXX}") not found. Do $(SUSUWU_SH_CODE_QUOTE "apt install clang") or $(SUSUWU_SH_CODE_QUOTE "apt install gcc")."
		exit 1
	fi
	LD="${CXX}"
	CC="${CXX}"
...
SUSUWU_PROCESS_RELEASE_DEBUG() { #/* Usage: `SUSUWU_PROCESS_RELEASE_DEBUG $@` [This processes params passed to `${0}`.] */
	if SUSUWU_SH_HAS_PARAM "--release" "$@"; then
		SUSUWU_PRINT "SUSUWU_PROCESS_RELEASE_DEBUG()" "$(SUSUWU_SH_NOTICE)" "$(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--release" "$@") $(SUSUWU_SH_CURRENT_VALUE "--release")") does not support profilers/debuggers (use $(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--release" "$@") $(SUSUWU_SH_PROPOSED_VALUE "--debug")") for this)."
		...
		if ! SUSUWU_SH_HAS_PARAM "--debug" "$@"; then
			SUSUWU_PRINT "SUSUWU_PROCESS_RELEASE_DEBUG()" "$(SUSUWU_SH_NOTICE)" "$(SUSUWU_SH_CODE_QUOTE "${0} $*") defaults to $(SUSUWU_SH_CODE_QUOTE "${0} $* $(SUSUWU_SH_CURRENT_VALUE "--debug")")."
		fi
		SUSUWU_PRINT "SUSUWU_PROCESS_RELEASE_DEBUG()" "$(SUSUWU_SH_NOTICE)" "$(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--debug" "$@") $(SUSUWU_SH_CURRENT_VALUE "--debug")") is slow (use $(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--debug" "$@") $(SUSUWU_SH_PROPOSED_VALUE "--release")") to improve how fast $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_PATH_UNAMBIGUOUS "\${BINDIR}/\${OUTPUT}")") executes)."
...
SUSUWU_SETUP_OBJDIR() { #/* Usage: `SUSUWU_SETUP_OBJDIR "./obj/"` */
	if [ -z "${OBJDIR}" ]; then
		OBJDIR="${1}"
		SUSUWU_SETUP_OBJDIR_OLD() ( SUSUWU_SH_PATH_QUOTE "$(SUSUWU_SH_CURRENT_VALUE "${OBJDIR}")" )
		SUSUWU_SETUP_OBJDIR_NEW() ( SUSUWU_SH_PATH_QUOTE "$(SUSUWU_SH_PROPOSED_VALUE "<new-path>")" )
		SUSUWU_PRINT "SUSUWU_SETUP_OBJDIR()" "$(SUSUWU_SH_NOTICE)" "To redirect $(SUSUWU_SH_CODE_QUOTE "${CXX} -c ... -o \"\${$(SUSUWU_SH_VAR "OBJDIR" "${SUSUWU_SH_BROWN}")}\${OBJ}.o\"") (which has $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_VAR "OBJDIR")=$(SUSUWU_SETUP_OBJDIR_OLD)")), execute $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_VAR "OBJDIR")=$(SUSUWU_SETUP_OBJDIR_NEW)") (where $(SUSUWU_SETUP_OBJDIR_NEW) is a directory which you choose)."
	else
		SUSUWU_PRINT "SUSUWU_SETUP_OBJDIR()" "$(SUSUWU_SH_NOTICE)" "$(SUSUWU_SH_CODE_QUOTE "${CXX} -c ... -o \"\${$(SUSUWU_SH_VAR "OBJDIR")}\"") inherits local $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_VAR "OBJDIR")=\"${OBJDIR}\"") until you execute $(SUSUWU_SH_CODE_QUOTE "unset OBJDIR")."
	fi
...
SUSUWU_SETUP_BINDIR() { #/* Usage: `SUSUWU_SETUP_BINDIR "./bin/"` */
	if [ -z "${BINDIR}" ]; then
		BINDIR="${1}"
		SUSUWU_SETUP_BINDIR_OLD() ( SUSUWU_SH_PATH_QUOTE "$(SUSUWU_SH_CURRENT_VALUE "${BINDIR}")" )
		SUSUWU_SETUP_BINDIR_NEW() ( SUSUWU_SH_PATH_QUOTE "$(SUSUWU_SH_PROPOSED_VALUE "<new-path>")" )
		SUSUWU_PRINT "SUSUWU_SETUP_BINDIR()" "$(SUSUWU_SH_NOTICE)" "To redirect $(SUSUWU_SH_CODE_QUOTE "${LD} ... -o \"\${$(SUSUWU_SH_VAR "BINDIR" "${SUSUWU_SH_BROWN}")}\${OUTPUT}\"") (which has $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_VAR "BINDIR")=$(SUSUWU_SETUP_BINDIR_OLD)")), execute $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_VAR "BINDIR")=$(SUSUWU_SETUP_BINDIR_NEW)") (where $(SUSUWU_SETUP_BINDIR_NEW) is a directory which you choose)."
	else
		SUSUWU_PRINT "SUSUWU_SETUP_BINDIR()" "$(SUSUWU_SH_NOTICE)" "$(SUSUWU_SH_CODE_QUOTE "${LD} ... -o \"\${$(SUSUWU_SH_VAR "BINDIR")}\${OUTPUT}\"") inherits local $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_VAR "BINDIR")=\"$(SUSUWU_SH_CURRENT_VALUE "${BINDIR}")\"") until you execute $(SUSUWU_SH_CODE_QUOTE "unset BINDIR")."
	fi
...
SUSUWU_SETUP_OUTPUT() { #/* Usage: `SUSUWU_SETUP_OUTPUT "YourProgram"` */
	if [ -z "${BINDIR%/}" ] || [ -z "${OBJDIR%/}" ]; then
		SUSUWU_PRINT "SUSUWU_SETUP_OUTPUT()" "$(SUSUWU_SH_ERROR)" "$(SUSUWU_SH_CODE_QUOTE "[ -z \"\${BINDIR%/}\" ] || [ \"-z \${OBJDIR%/}\" ]"): $(SUSUWU_SH_CODE_QUOTE "${0}") must call $(SUSUWU_SH_CODE_QUOTE "SUSUWU_SETUP_OBJDIR()") + $(SUSUWU_SH_CODE_QUOTE "SUSUWU_SETUP_BINDIR()") before $(SUSUWU_SH_CODE_QUOTE "SUSUWU_SETUP_OUTPUT()")."
...
SUSUWU_CLEAN_OUTPUT_IMPL() ( #/* Usage: `SUSUWU_CLEAN_OUTPUT_IMPL "Reason to clean" "postscript" */
	SUSUWU_PRINT "SUSUWU_CLEAN_OUTPUT_IMPL()" "$(SUSUWU_SH_INFO)" "${1}; will remove intermediate objects ($(SUSUWU_SH_CODE_QUOTE "rm \"${OBJDIR}\"*.o")), plus remove generated executables ($(SUSUWU_SH_CODE_QUOTE "rm \"${BINDIR}\"*.{exe,out}"))${2}"
...
SUSUWU_CLEAN_OUTPUT() { #/* Usage: `SUSUWU_REBUILD_OUTPUT "Reason to clean" */
	SUSUWU_CLEAN_OUTPUT_IMPL "${1}" ", plus exit. [Use $(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--clean") $(SUSUWU_SH_PROPOSED_VALUE "--rebuild")") to remove plus continue.]"
	exit 0
}
SUSUWU_REBUILD_OUTPUT() ( #/* Usage: `SUSUWU_REBUILD_OUTPUT "Reason to rebuild" */
	SUSUWU_CLEAN_OUTPUT_IMPL "${1}" ", plus continue. [Use $(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--rebuild") $(SUSUWU_SH_PROPOSED_VALUE "--clean")") to remove plus exit.]"
)
SUSUWU_PROCESS_CLEAN_REBUILD() { #/* Usage: `SUSUWU_PROCESS_CLEAN_REBUILD $@` [This processes params passed to `${0}`.] */
	if SUSUWU_SH_HAS_PARAM "--clean" "$@"; then
		SUSUWU_CLEAN_OUTPUT "Was called as $(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--clean" "$@") $(SUSUWU_SH_CURRENT_VALUE "--clean")")"
	fi
	if SUSUWU_SH_HAS_PARAM "--rebuild" "$@"; then
		SUSUWU_REBUILD_OUTPUT "Was called as $(SUSUWU_SH_CODE_QUOTE "${0} $(SUSUWU_SH_REMOVE_PARAM "--rebuild" "$@") $(SUSUWU_SH_CURRENT_VALUE "--rebuild")")"
	fi
}
...
#shellcheck disable=SC2068
	for LOCAL_SOURCE in $@; do
		...
		if [ -n "$(find "${LOCAL_SOURCE}" -newer "${LOCAL_OBJECT}" 2>/dev/null)" ] && [ -e "${LOCAL_SOURCE}" ]; then #/* `-n`: not nil, `-e`: exists. */
			SUSUWU_REBUILD_OUTPUT "$(SUSUWU_SH_PATH_QUOTE "${LOCAL_SOURCE}") (which is a common $(SUSUWU_SH_CODE_QUOTE "#include")) is newer than $(SUSUWU_SH_PATH_CODE "${LOCAL_OBJECT}")"
			break
...
SUSUWU_BUILD_CTAGS() ( #/* Usage: `SUSUWU_BUILD_CTAGS [-flags... --flags...] [SOURCE_DIR]...`. Return value: if `ctags` is called; `0`, if not; `1`. */
	...
	if command -v ctags >/dev/null; then
		if [ -z "${1}" ] || [ -z "${2}" ]; then
			...
			SUSUWU_PRINT "SUSUWU_BUILD_CTAGS()" "$(SUSUWU_SH_INFO)" "Was called with less than 2 params; will default to $(SUSUWU_SH_CODE_QUOTE "SUSUWU_BUILD_CTAGS ${CTAGS_DEFAULTS}")."
...
#shellcheck disable=SC2086
		...
		SUSUWU_PRINT "SUSUWU_BUILD_CTAGS()" "$(SUSUWU_SH_ERROR)" "\"ctags not found\"; do $(SUSUWU_SH_CODE_QUOTE "apt install ctags")."
	fi
...
#shellcheck disable=SC2046 #Is not possible to use more quotes.
	...
	if [ false = ${BUILDNEW} ]; then
		SUSUWU_STATUS=0
		SUSUWU_PRINT "SUSUWU_BUILD_EXECUTABLE()" "$(SUSUWU_SH_SUCCESS)" "Reused $(SUSUWU_SH_PATH_QUOTE "${BINDIR}${OUTPUT}") ($(stat -c%s "${BINDIR}${OUTPUT}") bytes)."
	elif [ 0 -eq ${SUSUWU_STATUS} ]; then
		SUSUWU_PRINT "SUSUWU_BUILD_EXECUTABLE()" "$(SUSUWU_SH_SUCCESS)" "Produced $(SUSUWU_SH_PATH_QUOTE "${BINDIR}${OUTPUT}") ($(stat -c%s "${BINDIR}${OUTPUT}") bytes)."
	else
		SUSUWU_PRINT "SUSUWU_BUILD_EXECUTABLE()" "$(SUSUWU_SH_ERROR)" "$(SUSUWU_SH_CODE_QUOTE "${LD}") returned status code $(SUSUWU_SH_STATUS_CODE "${SUSUWU_STATUS}"). [If errors include \"ld... $(SUSUWU_SH_ERROR_MESSAGE "unknown file type")\" or \"ld... $(SUSUWU_SH_ERROR_MESSAGE "undefined symbol __asan_")*\"; remove intermediate objects ($(SUSUWU_SH_CODE_QUOTE "rm \"${OBJDIR}\"*.o")). Use $(SUSUWU_SH_CODE_QUOTE "${0} ${SUSUWU_SH_CONSOLE_PARAMS} $(SUSUWU_SH_PROPOSED_VALUE "--rebuild")") to remove plus continue."
	fi
	return ${SUSUWU_STATUS}
}
	...
	if [ 0 -eq ${SUSUWU_STATUS} ] || [ false = ${BUILDNEW} ]; then
		if [ -z "${CROSS_COMP}" ]; then #/* `if("" == CROSS_COMP)` */
			...
			if ! command -v wine >/dev/null; then
				SUSUWU_PRINT "SUSUWU_TEST_OUTPUT()" "$(SUSUWU_SH_INFO)" "$(SUSUWU_SH_CODE_QUOTE "wine not found"). do $(SUSUWU_SH_CODE_QUOTE "apt install wine")."
				return 1
			fi
			if [ true = ${SUSUWU_S} ]; then
		...
		if [ 0 -eq ${SUSUWU_STATUS} ]; then
			SUSUWU_PRINT "SUSUWU_TEST_OUTPUT()" "$(SUSUWU_SH_SUCCESS)" "$(SUSUWU_SH_CODE_QUOTE "${BINDIR}${OUTPUT}") returned status SusuwuUnitTestsBitmask($(SUSUWU_SH_STATUS_CODE "${SUSUWU_STATUS}"))."
		else
			SUSUWU_PRINT "SUSUWU_TEST_OUTPUT()" "$(SUSUWU_SH_ERROR)" "$(SUSUWU_SH_CODE_QUOTE "${BINDIR}${OUTPUT}") returned status SusuwuUnitTestsBitmask($(SUSUWU_SH_STATUS_CODE "${SUSUWU_STATUS}"))."
		fi
	fi
	return ${SUSUWU_STATUS}
}
...
SUSUWU_TODO_LIST() ( #/* Usage: `SUSUWU_TODO_LIST "NameOfTool" "what to expect from future versions"` */
	SUSUWU_PRINT "SUSUWU_TODO_LIST()" "$(SUSUWU_SH_ERROR)" "$(SUSUWU_SH_CODE_QUOTE "${1}") ${2} is on a long \"TODO\" list. If you want this soon; you can [contribute](https://github.com/SwuduSusuwu/SubStack/#Contributor-conventionsrules), or [ask for this](https://github.com/SwuduSusuwu/SubStack/issues/new)."
	exit 1
)
SUSUWU_FORMAT() ( #/* Is analogous to `make format`. */
	SUSUWU_TODO_LIST "SUSUWU_FORMAT()" "(which will use tools such as $(SUSUWU_SH_CODE_QUOTE "prettier") or $(SUSUWU_SH_CODE_QUOTE "clang-format"))"
)
SUSUWU_DOCS() ( #/* Is analogous to `make docs`. */
	SUSUWU_TODO_LIST "SUSUWU_DOCS()" "(which will use tools such as $(SUSUWU_SH_CODE_QUOTE "sphinx") or $(SUSUWU_SH_CODE_QUOTE "doxygen"))"
)
SUSUWU_HAS_USABLE_USRBIN() ( #/* Usage: `[ 0 -eq $(SUSUWU_HAS_USABLE_USRBIN("${USRBIN}")) ]`. Is just for internal use. */
	...
	if [ -z "${PATH}" ]; then
		SUSUWU_PRINT "SUSUWU_HAS_USABLE_USRBIN()" "$(SUSUWU_SH_WARNING)" "$(SUSUWU_SH_CODE_QUOTE "\${PATH}") not set; will use \`$(SUSUWU_SH_VAR "USRBIN")=\"$(SUSUWU_SH_PROPOSED_VALUE "${1}")\"\` (first guessed path which exists)."
		return 0
	fi
...
SUSUWU_PROCESS_USRBIN() { #/* Usage: `SUSUWU_USRBIN ["SUSUWU_[UN]INSTALL"] ["/usr/bin"...]`. Is just for internal use. */
	if [ -n "${2}" ]; then #/* function `${1}` was passed USRBIN as its first param (our second param) */
		...
		if [ -d "${USRBIN}" ]; then
			...
			SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_WARNING)" "$(SUSUWU_SH_CODE_QUOTE "${1}") was passed \`$(SUSUWU_SH_VAR "USRBIN")=\"$(SUSUWU_SH_CURRENT_VALUE "${USRBIN}")\"\` (which is not a directory); will autodetect \`$(SUSUWU_SH_VAR "USRBIN")\`."
		fi
	fi
	...
	if [ -z "${USRBIN}" ]; then #if array above is "", or does not have usable values
		SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_NOTICE)" "None of the usual paths for \`$(SUSUWU_SH_VAR "USRBIN")\` suitable to use; will use the first directory from \`$(SUSUWU_SH_VAR "\${PATH}")\` which exists."
		USRBIN="$(SUSUWU_FIRST_PATH)"
	fi
	USRBIN="$(SUSUWU_PATH_UNAMBIGUOUS "${USRBIN}")"
	if [ -d "${USRBIN}" ]; then
		SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_NOTICE)" "Have set \`$(SUSUWU_SH_VAR "USRBIN")=\"$(SUSUWU_SH_PROPOSED_VALUE "${USRBIN}")\"\`, since $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_FUNCTION "${1}")") was not passed \`$(SUSUWU_SH_VAR "USRBIN")\`."
		return 0
	fi
	SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_ERROR)" "Failed to autodetect \`$(SUSUWU_SH_VAR "USRBIN")\` (which $(SUSUWU_SH_CODE_QUOTE "$(SUSUWU_SH_FUNCTION "${1}")") uses)."
