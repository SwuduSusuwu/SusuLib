...
SUSUWU_SETUP_OBJDIR() { #/* Usage: `SUSUWU_SETUP_OBJDIR "./obj/"` */
	if [ -z ${OBJDIR} ]; then
		OBJDIR="${1}"
		SUSUWU_SETUP_OBJDIR_OLD="$(SUSUWU_SH_QUOTE "PATH" "$(SUSUWU_SH_QUOTE "CURRENT" "${OBJDIR}")")"
		SUSUWU_SETUP_OBJDIR_NEW="$(SUSUWU_SH_QUOTE "PATH" "$(SUSUWU_SH_QUOTE "PROPOSED" "<new-path>")")"
		SUSUWU_PRINT "SUSUWU_SETUP_OBJDIR()" "$(SUSUWU_SH_NOTICE)" "To redirect $(SUSUWU_SH_QUOTE "CODE" "${CXX} -c ... -o \"\${$(SUSUWU_SH_QUOTE "VAR" "OBJDIR")}\${OBJ}.o\"") (which has $(SUSUWU_SH_QUOTE "CODE" "$(SUSUWU_SH_QUOTE "VAR" "OBJDIR")=${SUSUWU_SETUP_OBJDIR_OLD}")), execute $(SUSUWU_SH_QUOTE "CODE" "$(SUSUWU_SH_QUOTE "VAR" "OBJDIR")=${SUSUWU_SETUP_OBJDIR_NEW}") (where ${SUSUWU_SETUP_OBJDIR_NEW} is a directory which you choose)."
	else
...
SUSUWU_PROCESS_USRBIN() { #/* Usage: `SUSUWU_USRBIN ["SUSUWU_[UN]INSTALL"] ["/usr/bin"...]`. Is just for internal use. */
	...
	USRBIN="$(SUSUWU_PATH_UNAMBIGUOUS "${USRBIN}")"
	if [ -d "${USRBIN}" ]; then
		SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_NOTICE)" "Have set \`$(SUSUWU_SH_QUOTE "VAR" "USRBIN")=\"$(SUSUWU_SH_QUOTE "PROPOSED" "${USRBIN}")\"\`, since $(SUSUWU_SH_QUOTE "CODE" "$(SUSUWU_SH_QUOTE "FUNCTION" "${1}")") was not passed \`$(SUSUWU_SH_QUOTE "VAR" "USRBIN")\`."
		return 0
	fi
	SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_ERROR)" "Failed to autodetect \`$(SUSUWU_SH_QUOTE "VAR" "USRBIN")\` (which $(SUSUWU_SH_QUOTE "CODE" "$(SUSUWU_SH_QUOTE "FUNCTION" "${1}")") uses)."
