...
SUSUWU_PROCESS_RELEASE_DEBUG() { #/* Usage: `SUSUWU_PROCESS_RELEASE_DEBUG $@` [This processes params passed to `${0}`.] */
	if SUSUWU_SH_HAS_PARAM "--release" "$@"; then
		SUSUWU_PRINT "SUSUWU_PROCESS_RELEASE_DEBUG()" "$(SUSUWU_SH_NOTICE)" "\`${0} $(SUSUWU_SH_REMOVE_PARAM "--release" "$@") $(SUSUWU_SH_CURRENT_VALUE "--release")\` does not support profilers/debuggers (use \`${0} $(SUSUWU_SH_REMOVE_PARAM "--release" "$@") $(SUSUWU_SH_PROPOSED_VALUE "--debug")\` for this)."
		...
		if ! SUSUWU_SH_HAS_PARAM "--debug" "$@"; then
			SUSUWU_PRINT "SUSUWU_PROCESS_RELEASE_DEBUG()" "$(SUSUWU_SH_NOTICE)" "\`${0} $*\` defaults to \`${0} $* $(SUSUWU_SH_CURRENT_VALUE "--debug")\`."
		fi
		SUSUWU_PRINT "SUSUWU_PROCESS_RELEASE_DEBUG()" "$(SUSUWU_SH_NOTICE)" "\`${0} $(SUSUWU_SH_REMOVE_PARAM "--debug" "$@") $(SUSUWU_SH_CURRENT_VALUE "--debug")\` is slow (use \`${0} $(SUSUWU_SH_REMOVE_PARAM "--debug" "$@") $(SUSUWU_SH_PROPOSED_VALUE "--release")\` to improve how fast \`$(SUSUWU_PATH_UNAMBIGUOUS "\${BINDIR}/\${OUTPUT}")\` executes)."
...
SUSUWU_SETUP_OBJDIR() { #/* Usage: `SUSUWU_SETUP_OBJDIR "./obj/"` */
	if [ -z "${OBJDIR}" ]; then
		OBJDIR="${1}"
		SUSUWU_SETUP_OBJDIR_OLD() (echo "\"$(SUSUWU_SH_CURRENT_VALUE "${OBJDIR}")\"")
		SUSUWU_SETUP_OBJDIR_NEW() (echo "\"$(SUSUWU_SH_PROPOSED_VALUE "./objnew/")\"")
		SUSUWU_PRINT "SUSUWU_SETUP_OBJDIR()" "$(SUSUWU_SH_NOTICE)" "To redirect \`${CXX} -c ... -o \"\${$(SUSUWU_SH_VAR "OBJDIR")}\${OBJ}.o\"\` (which has \`$(SUSUWU_SH_VAR "OBJDIR")=$(SUSUWU_SETUP_OBJDIR_OLD)\`), execute \`$(SUSUWU_SH_VAR "OBJDIR")=$(SUSUWU_SETUP_OBJDIR_NEW)\` (where $(SUSUWU_SETUP_OBJDIR_NEW) is a directory which you choose)."
	else
		SUSUWU_PRINT "SUSUWU_SETUP_OBJDIR()" "$(SUSUWU_SH_NOTICE)" "\`${CXX} -c ... -o \"\${$(SUSUWU_SH_VAR "OBJDIR")}\"\` inherits local \`$(SUSUWU_SH_VAR "OBJDIR")=$(SUSUWU_SH_CURRENT_VALUE "${OBJDIR}")\` until you execute \`unset OBJDIR\`."
	fi
...
SUSUWU_SETUP_BINDIR() { #/* Usage: `SUSUWU_SETUP_BINDIR "./bin/"` */
	if [ -z "${BINDIR}" ]; then
		BINDIR="${1}"
		SUSUWU_SETUP_BINDIR_OLD() (echo "\"$(SUSUWU_SH_CURRENT_VALUE "${BINDIR}")\"")
		SUSUWU_SETUP_BINDIR_NEW() (echo "\"$(SUSUWU_SH_PROPOSED_VALUE "./binnew/")\"")
		SUSUWU_PRINT "SUSUWU_SETUP_BINDIR()" "$(SUSUWU_SH_NOTICE)" "To redirect \`${LD} ... -o \"\${$(SUSUWU_SH_VAR "BINDIR")}\${OUTPUT}\"\` (which has \`$(SUSUWU_SH_VAR "BINDIR")=$(SUSUWU_SETUP_BINDIR_OLD)\`), execute \`$(SUSUWU_SH_VAR "BINDIR")=$(SUSUWU_SETUP_BINDIR_NEW)\` (where $(SUSUWU_SETUP_BINDIR_NEW) is a directory which you choose)."
	else
		SUSUWU_PRINT "SUSUWU_SETUP_BINDIR()" "$(SUSUWU_SH_NOTICE)" "\`${LD} ... -o \"\${$(SUSUWU_SH_USE2 "${SUSUWU_SH_LIGHT_CYAN}" "BINDIR")}\${OUTPUT}\"\` inherits local \`$(SUSUWU_SH_VAR "BINDIR")=$(SUSUWU_SH_CURRENT_VALUE "${BINDIR}")\` until you execute \`unset BINDIR\`."
	fi
...
SUSUWU_CLEAN_OUTPUT() { #/* Usage: `SUSUWU_REBUILD_OUTPUT "Reason to clean" */
	SUSUWU_CLEAN_OUTPUT_IMPL "${1}" ", plus exit. [Use \`${0} $(SUSUWU_SH_REMOVE_PARAM "--clean") $(SUSUWU_SH_PROPOSED_VALUE "--rebuild")\` to remove plus continue.]"
	exit 0
}
SUSUWU_REBUILD_OUTPUT() ( #/* Usage: `SUSUWU_REBUILD_OUTPUT "Reason to rebuild" */
	SUSUWU_CLEAN_OUTPUT_IMPL "${1}" ", plus continue. [Use \`${0} $(SUSUWU_SH_REMOVE_PARAM "--rebuild") $(SUSUWU_SH_PROPOSED_VALUE "--clean")\` to remove plus exit.]"
)
SUSUWU_PROCESS_CLEAN_REBUILD() { #/* Usage: `SUSUWU_PROCESS_CLEAN_REBUILD $@` [This processes params passed to `${0}`.] */
	if SUSUWU_SH_HAS_PARAM "--clean" "$@"; then
		SUSUWU_CLEAN_OUTPUT "Was called as \`${0} $(SUSUWU_SH_REMOVE_PARAM "--clean" "$@") $(SUSUWU_SH_CURRENT_VALUE "--clean")\`"
	fi
	if SUSUWU_SH_HAS_PARAM "--rebuild" "$@"; then
		SUSUWU_REBUILD_OUTPUT "Was called as \`${0} $(SUSUWU_SH_REMOVE_PARAM "--rebuild" "$@") $(SUSUWU_SH_CURRENT_VALUE "--rebuild")\`"
	fi
}
...
#shellcheck disable=SC2046 #Is not possible to use more quotes.
	...
	if [ false = ${BUILDNEW} ]; then
		...
		SUSUWU_PRINT "SUSUWU_BUILD_EXECUTABLE()" "$(SUSUWU_SH_ERROR)" "\`${LD}\` returned status code $(SUSUWU_SH_STATUS_CODE "${SUSUWU_STATUS}"). [If errors include \"ld... $(SUSUWU_SH_ERROR_MESSAGE "unknown file type")\" or \"ld... $(SUSUWU_SH_ERROR_MESSAGE "undefined symbol __asan_")*\"; remove intermediate objects (\`rm \"${OBJDIR}\"*.o\`). Use \`${0} ${SUSUWU_SH_CONSOLE_PARAMS} $(SUSUWU_SH_PROPOSED_VALUE "--rebuild")\` to remove plus continue."
	fi
	return ${SUSUWU_STATUS}
}
...
SUSUWU_TEST_OUTPUT() { #/* Usage: ... `SUSUWU_BUILD_EXECUTABLE && SUSUWU_TEST_OUTPUT [<silent-execution>]; exit $?` */
	if [ 0 -eq ${SUSUWU_STATUS} ] || [ false = ${BUILDNEW} ]; then
		...
		if [ 0 -eq ${SUSUWU_STATUS} ]; then
			SUSUWU_PRINT "SUSUWU_TEST_OUTPUT()" "$(SUSUWU_SH_SUCCESS)" "\`${BINDIR}${OUTPUT}\` returned status SusuwuUnitTestsBitmask($(SUSUWU_SH_STATUS_CODE "${SUSUWU_STATUS}"))."
		else
			SUSUWU_PRINT "SUSUWU_TEST_OUTPUT()" "$(SUSUWU_SH_ERROR)" "\`${BINDIR}${OUTPUT}\` returned status SusuwuUnitTestsBitmask($(SUSUWU_SH_STATUS_CODE "${SUSUWU_STATUS}"))."
		fi
	fi
	return ${SUSUWU_STATUS}
}
...
SUSUWU_HAS_USABLE_USRBIN() ( #/* Usage: `[ 0 -eq $(SUSUWU_HAS_USABLE_USRBIN("${USRBIN}")) ]`. Is just for internal use. */
	...
	if [ -z "${PATH}" ]; then
		SUSUWU_PRINT "SUSUWU_HAS_USABLE_USRBIN()" "$(SUSUWU_SH_WARNING)" "\`\${PATH}\` not set; will use \`$(SUSUWU_SH_VAR "USRBIN")=\"$(SUSUWU_SH_PROPOSED_VALUE "${1}")\"\` (first guessed path which exists)."
		return 0
	fi
...
SUSUWU_PROCESS_USRBIN() { #/* Usage: `SUSUWU_USRBIN ["SUSUWU_[UN]INSTALL"] ["/usr/bin"...]`. Is just for internal use. */
	if [ -n "${2}" ]; then #/* function `${1}` was passed USRBIN as its first param (our second param) */
		...
		if [ -d "${USRBIN}" ]; then
			...
			SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_WARNING)" "\`${1}\` was passed \`$(SUSUWU_SH_VAR "USRBIN")=\"$(SUSUWU_SH_CURRENT_VALUE "${USRBIN}")\"\` (which is not a directory); will autodetect \`$(SUSUWU_SH_VAR "USRBIN")\`."
		fi
	fi
	...
	if [ -z "${USRBIN}" ]; then #if array above is "", or does not have usable values
		SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_NOTICE)" "None of the usual paths for \`$(SUSUWU_SH_VAR "USRBIN")\` suitable to use; will use the first directory from \`\${PATH}\` which exists."
		USRBIN="$(SUSUWU_FIRST_PATH)"
	fi
	USRBIN="$(SUSUWU_PATH_UNAMBIGUOUS "${USRBIN}")"
	if [ -d "${USRBIN}" ]; then
		SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_NOTICE)" "Have set \`$(SUSUWU_SH_VAR "USRBIN")=\"$(SUSUWU_SH_PROPOSED_VALUE "${USRBIN}")\"\`, since \`${1}\` was not passed \`$(SUSUWU_SH_VAR "USRBIN")\`."
		return 0
	fi
	SUSUWU_PRINT "SUSUWU_PROCESS_USRBIN()" "$(SUSUWU_SH_ERROR)" "Failed to autodetect \`$(SUSUWU_SH_VAR "USRBIN")\` (which \`${1}\` uses)."
