...
SUSUWU_SH_COLOR_COUNT() ( #/* Usage: `LOCAL_COLOR_COUNT=SUSUWU_SH_COLOR_COUNT` */
	...
	if [ "${TERM}" = "" ]; then
		echo "[$0: Warning: SUSUWU_SH_COLOR_COUNT(): If your console (\`[ \"\${TERM}\" = \"${TERM}\" ]\`, which _GitHub_ uses) lacks colors such as ${SUSUWU_SH_BLUE}blue${SUSUWU_SH_DEFAULT} (shows glitches, or literal codes such as \"\\\033[0;34m\"), use \`export TERM=\"dumb\"\` to disable those.]" >&2
...
SUSUWU_SH_QUOTE_SUB() { # Usage: `SUSUWU_SH_QUOTE_SUB <type-of-quote> <code | quote>`
	case "${1}" in #/* Notice: update [`README.md#cc-source`](README.md#cc-source) if you update those. */
		...
		"PATH") #path to file or directory
			echo "\"$(SUSUWU_SH_USE2 "${SUSUWU_SH_PURPLE}" "${2}")\"" ;;
		"FUNCTION") #name of function
			SUSUWU_SH_USE2 "${SUSUWU_SH_LIGHT_CYAN}" "${2}" ;;
		"STDERR") #error message quote
			SUSUWU_SH_USE2 "${SUSUWU_SH_RED}" "${2}" ;;
		"STATUS") #status code / return value
			SUSUWU_SH_USE2 "${SUSUWU_SH_PURPLE}" "${2}" ;;
		"VAR") #name of variable / name of constant
			SUSUWU_SH_USE2 "${SUSUWU_SH_LIGHT_CYAN}" "${2}" ;;
		"CURRENT") #current value
			SUSUWU_SH_USE2 "${SUSUWU_SH_GREEN}" "${2}" ;;
		"PROPOSED") #speculative / proposed value
			SUSUWU_SH_USE2 "${SUSUWU_SH_LIGHT_PURPLE}" "${2}" ;;
	esac
	return $?
}
