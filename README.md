...
*Notice*: You switched to the [_experimental_](https://github.com/SwuduSusuwu/SubStack/blob/experimental/) branch, which has the newest features, but is unstable, and has much use of `git rebase` force pushes (which require you to use `git pull --rebase`); use [_trunk_ branch](https://github.com/SwuduSusuwu/SubStack/blob/trunk/README.md#table-of-contents) (`git switch trunk`) for code which is more stable plus has more support.
...
- [`./hooks/post-checkout`](./hooks/post-checkout) is custom `post-checkout` (produces `./tags`, uses `SUSUWU_LOCAL_WORKSPACE_JSON` to allow `clang-tidy`)
- [`./hooks/post-merge`](./hooks/post-merge) is custom `post-merge` (produces `./tags`, uses `SUSUWU_LOCAL_WORKSPACE_JSON` to allow `clang-tidy`.)
...
- [`./hooks/prepare-commit-message`](./hooks/prepare-commit-message) is [custom `prepare-commit-msg`](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks) (`git add ./tags` if tracked, + `SUSUWU_GIHUB_WORKSPACE_JSON && git add ./compile_commands.json`.)
