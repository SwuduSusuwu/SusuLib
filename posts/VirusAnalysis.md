...
To test certificates, view [`../README.md#signaturecertificate`](../README.md#signatures--certificates) + [this post](https://swudususuwu.substack.com/p/s256_kz1xw3wo0bpceuvl3qh4nulf0hnll4nnkehiko6zk0).
...
#	ifdef ASSISTANTCNS_LIMIT_TO_TOP_LEVEL
		for(pugi::xml_node node = doc.child("html").child("body").child("a"); node; node = node.next_sibling("a")) {
			const auto &attributeHref = node.attribute("href"); /* notice: if multiple execution cores access `doc`, remove `&` (do not reuse references) unless you know other execution cores just use `const` member functions */
			if(attributeHref) {
